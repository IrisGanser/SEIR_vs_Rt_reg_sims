...
setwd("~/PhD/COVID_France/SEIR_vs_Rt_sims/boot_sim_2params_all_new2_Simulx_SEIRAHD")
...
for(j in 1:100){
  ...
  for(i in 1:100){
    rm(popparams)
    try(popparams <- read.table(paste0(getwd(), "/SEIRAHD_Simulx_new2_2params_", j, "boot", i, "/populationParameters.txt"), 
                                header = TRUE, sep = ",") %>%
  ...
  point_est <- read.table(paste0(dir3, "/SEIRAHD_Simulx_new2_pe_", j, "/populationParameters.txt"), 
                          header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j) %>%
  ...
  SEIRAHD_boot_list_all[[j]] <- pop_params_boot %>%
    ungroup() %>%
...
save(SEIRAHD_boot_list_all, file = "SEIRAHD_boot_list_2params_new2_Simulx.RData")



# join point estimates and pivot into longer format 
SEIRAHD_boot_df_long <- do.call("rbind.data.frame", SEIRAHD_boot_list_all) %>%
...
ggplot(SEIRAHD_boot_df_long, aes(ymin = CI_LL, ymax = CI_UL, x = sim_rep, y = mean_est, col = method)) + 
  ...
  labs(title= "Bootstrap SEIRAHD Simulx new2", 
