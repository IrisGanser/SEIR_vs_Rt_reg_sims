...
setwd("~/PhD/COVID_France/SEIR_vs_Rt_sims/boot_sim_2params_ABM6")
...
for(j in 1:100){
  ...
  for(i in 1:100){
    rm(popparams)
    try(popparams <- read.table(paste0(getwd(), "/SEIR_ABM_2params_hybrid6_", j, "boot", i, "/populationParameters.txt"), 
                                header = TRUE, sep = ",") %>%
  ...
  pop_params_boot <- bootstrap_CI_calc(params_sim_rep) %>%
      mutate(model = "hybrid ABM 6")
  
  # load point estimates
  point_est <- read.table(paste0(dir3, "/ABM_hybrid6_pe_", j, "/populationParameters.txt"), 
                          header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j) %>%
  ...
  ABM6_hybrid_list[[j]] <- pop_params_boot %>%
    ungroup() %>%
...
save(ABM6_hybrid_list, file = "ABM6_hybrid_list.RData")
...
for(j in 1:100){
  ...
  for(i in 1:100){
    rm(popparams)
    try(popparams <- read.table(paste0(getwd(), "/SEIR_ABM_2params_rm6_", j, "boot", i, "/populationParameters.txt"), 
                                header = TRUE, sep = ",") %>%
          bootstrap_cleaning(., boot_rep = i, sim_rep = j))
  ...
  # calculate CIs
  pop_params_boot <- bootstrap_CI_calc(params_sim_rep) %>%
      mutate(model = "rm ABM 6")
  
  # load point estimates
  point_est <- read.table(paste0(dir3, "/ABM_rm6_pe_", j, "/populationParameters.txt"), 
                          header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j) %>%
    rename(mean_est2 = value)
  
  ABM6_rm_list[[j]] <- pop_params_boot %>%
    ungroup() %>%
    left_join(point_est, by = c("parameter", "sim_rep")) %>%
    select(parameter, sim_rep, model, mean_est1, mean_est2, CI_LL1, CI_LL2, CI_UL1, CI_UL2, sd_est)
...
save(ABM6_rm_list, file = "ABM6_rm_list.RData")
...
setdiff(full_list, sim_checklist_df) %>%
  ...
  print(n = Inf)
        ...
        # summary
load("ABM6_hybrid_list.RData")
load("ABM6_rm_list.RData")

ABM6_boot_df_comp <- bootstrap_summary(ABM6_rm_list, true_val_NPI1 = -1.45, true_val_NPI2 = -0.8) %>% 
  bind_rows(bootstrap_summary(ABM6_hybrid_list, true_val_NPI1 = -1.45, true_val_NPI2 = -0.8))

ggplot(ABM6_boot_df_comp, aes(ymin = CI_LL2, ymax = CI_UL2, x = sim_rep, y = mean_est2, col = model)) + 
...
ABM6_boot_df_comp_long <- ABM6_boot_df_comp %>%
  select(1:9) %>%
...
ggplot(ABM6_boot_df_comp_long, aes(ymin = CI_LL, ymax = CI_UL, x = sim_rep, y = mean_est, col = method)) + 
  geom_pointrange(position = position_dodge(width = 0.5)) + 
  scale_x_continuous(expand = c(0.01, 0.01)) + 
