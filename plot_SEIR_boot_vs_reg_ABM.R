...
dir5 <- "~/PhD/COVID_France/SEIR_vs_Rt_sims/boot_sim_2params_ABM7"
dir6 <- "~/PhD/COVID_France/SEIR_vs_Rt_sims/boot_sim_2params_ABM10"
...
#### ABM 7 ####
# load data
load(paste0(dir5, "/ABM7_hybrid_list.RData"))
load(paste0(dir5, "/ABM7_rm_list.RData"))

load(paste0(dir, "/reg_res_I_2params_all_ABM_rm7_df.RData"))
load(paste0(dir, "/reg_res_I_2params_all_ABM_hybrid7_df.RData"))



# bring data into right format and assemble
boot_df_ABM7 <- bootstrap_summary(ABM7_rm_list, true_val_NPI2 = -0.5) %>% 
  mutate(model = "SEIR ABM rm") %>%
  bind_rows(bootstrap_summary(ABM7_hybrid_list, true_val_NPI2 = -0.5) %>%
              mutate(model = "SEIR ABM hybrid")) %>%
  rename(mean_est = mean_est2, CI_LL = CI_LL2, CI_UL = CI_UL2)


reg_res_2params_ABM7 <- reg_res_I_2params_all_ABM_hybrid7_df %>%
  mutate(model = "Regression model ABM hybrid") %>%
  bind_rows(reg_res_I_2params_all_ABM_rm7_df %>% mutate(model = "Regression model ABM random mixing"))  %>%
  reg_summary(true_val_NPI2 = -0.5)

comp_df_ABM7 <- boot_df_ABM7 %>%
  bind_rows(reg_res_2params_ABM7 %>% rename(sim_rep = rep, mean_est = value))

# plot
pg_palette <- diverging_hcl("Purple-Green", n = 20)
comp_cols <- c(pg_palette[c(18, 14)], "black", pg_palette[c(3, 6)])

ggplot(comp_df_ABM7, aes(x = sim_rep, y = mean_est, ymin = CI_LL, ymax = CI_UL, col = model)) + 
  geom_pointrange(position = position_dodge(width = 0.8)) +
  geom_line(aes(y = true_value), col = "black", linetype = "dashed", linewidth = 0.8) + 
  scale_x_continuous(breaks = c(1, seq(10, 100, 10)), expand = c(0.01, 0.01)) + 
  facet_wrap(~parameter, scales = "free_y", nrow = 2) +
...
ggsave("SEIR_vs_reg_ABM7.jpeg", dpi = 400, width = 14, height = 8.5)

# bring into right format for metrics table
metric_df_ABM7 <- comp_df_ABM7 %>%
  select(parameter, model, perc_CI_covers, mean_bias, mean_rel_bias) %>%
  unique() %>%
  rename(coverage = perc_CI_covers, bias = mean_bias, relbias = mean_rel_bias) %>%
  pivot_longer(cols = c(coverage, bias, relbias), 
...
metric_df_ABM7  %>% 
  ungroup() %>%
  select(-parameter) %>%
  kable(digits = 2, format = "latex"#, table.attr = "style='width:62%;'"
        ) %>%
  kable_styling(bootstrap_options = "striped") %>%
  pack_rows("NPI 1", 1, 3) %>%
  pack_rows("NPI 2", 4, 6)
...
#### ABM 10 ####
# load data
load(paste0(dir6, "/ABM10_hybrid_nc_list.RData"))
load(paste0(dir6, "/ABM10_rm_nc_list.RData"))

load(paste0(dir, "/reg_res_I_2params_all_ABM_rm10_df.RData"))
load(paste0(dir, "/reg_res_I_2params_all_ABM_hybrid10_df.RData"))
...
reg_res_2params_ABM10 <- reg_res_I_2params_all_ABM_hybrid10_df %>%
  mutate(model = "Regression model ABM hybrid") %>%
  bind_rows(reg_res_I_2params_all_ABM_rm10_df %>% mutate(model = "Regression model ABM random mixing"))  %>%
  reg_summary(true_val_NPI2 = -0.8)

comp_df_ABM10 <- boot_df_ABM10 %>%
  bind_rows(reg_res_2params_ABM10 %>% rename(sim_rep = rep, mean_est = value))

# plot
pg_palette <- diverging_hcl("Purple-Green", n = 20)
comp_cols <- c(pg_palette[c(18, 14)], "black", pg_palette[c(3, 6)])

ggplot(comp_df_ABM10, aes(x = sim_rep, y = mean_est, ymin = CI_LL, ymax = CI_UL, col = model)) + 
  geom_pointrange(position = position_dodge(width = 0.8)) +
  geom_line(aes(y = true_value), col = "black", linetype = "dashed", linewidth = 0.8) + 
  scale_x_continuous(breaks = c(1, seq(10, 100, 10)), expand = c(0.01, 0.01)) + 
  facet_wrap(~parameter, scales = "free_y", nrow = 2) +
...
ggsave("SEIR_vs_reg_ABM10_wo_title.jpeg", dpi = 400, width = 14, height = 8.5)

# bring into right format for metrics table
metric_df_ABM10 <- comp_df_ABM10 %>%
  select(parameter, model, perc_CI_covers, mean_bias, mean_rel_bias) %>%
  unique() %>%
  rename(coverage = perc_CI_covers, bias = mean_bias, relbias = mean_rel_bias) %>%
  pivot_longer(cols = c(coverage, bias, relbias), 
