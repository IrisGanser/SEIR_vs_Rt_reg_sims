...
ggplot(res_EE_ld1_start_df3 %>% filter(dept_id %in% selected_depts[c(1:3, 5:7)]),
...
ggplot(res_EE_ld1_start_high_df3 %>% filter(dept_id %in% selected_depts[c(1:3, 5:7)]),
       aes(x = day, y = Rt, col = as.factor(ld1_start))) + 
  geom_line() +
  geom_ribbon(aes(ymax = CI_UL, ymin = CI_LL, fill = as.factor(ld1_start)), alpha = 0.3, colour = NA) +
  geom_line(aes(y = Rt_SEIRAHD), linetype = "dashed") + 
...
ggplot(res_EE_ld1_strength_df3 %>% filter(dept_id %in% selected_depts[1:3]),
...
# plots
ggplot(res_reg_ld1_start_df3, aes(ymin = CI_LL, ymax = CI_UL, x = ld1_start, 
...
ggplot(res_reg_ld1_start_high_df3, aes(ymin = CI_LL, ymax = CI_UL, x = ld1_start, 
                                      y = value, col = as.factor(ld1_start))) + 
  geom_pointrange(position = position_dodge(width = 1)) + 
  scale_x_continuous(expand = c(0.01, 0.01)) + 
  facet_wrap(~parameter, ncol = 1, scale = "free_y") +
...
ggplot(res_reg_ld1_strength_df3, aes(ymin = CI_LL, ymax = CI_UL, x = ld1_strength, 
