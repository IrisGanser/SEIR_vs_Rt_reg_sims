...
for(j in 1:100){
  ...
  for(i in 1:100){
    rm(popparams)
    try(popparams <- read.table(paste0(getwd(), "/SEIR_ABM_2params_hybrid10_nc_", j, "boot", i, "/populationParameters.txt"), 
                                header = TRUE, sep = ",") %>%
          bootstrap_cleaning(., boot_rep = i, sim_rep = j))
  ...
  # load point estimates
  try(point_est <- read.table(paste0(getwd(), "/ABM_hybrid10_pe_nc_", j, "/populationParameters.txt"), 
                              header = TRUE, sep = ",") %>%
        popparam_cleaning() %>%
        mutate(sim_rep = j) %>%
        rename(mean_est2 = value))
  ...
  ABM10_hybrid_nc_list[[j]] <- pop_params_boot %>%
    ungroup() %>%
    left_join(point_est, by = c("parameter", "sim_rep")) %>%
    select(parameter, sim_rep, model, mean_est1, mean_est2, CI_LL1, CI_LL2, CI_UL1, CI_UL2, sd_est)
...
save(ABM10_hybrid_nc_list, file = "ABM10_hybrid_nc_list.RData")
...
for(j in 1:15){
  ...
  for(i in 1:100){
    rm(popparams)
    try(popparams <- read.table(paste0(getwd(), "/SEIR_ABM_2params_rm10_nc_", j, "boot", i, "/populationParameters.txt"), 
                                header = TRUE, sep = ",") %>%
          bootstrap_cleaning(., boot_rep = i, sim_rep = j))
  ...
  # load point estimates
  point_est <- read.table(paste0(getwd(), "/ABM_rm10_pe_nc_", j, "/populationParameters.txt"), 
                          header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j) %>%
    rename(mean_est2 = value)
  
  ABM10_rm_nc_list[[j]] <- pop_params_boot %>%
    ungroup() %>%
    left_join(point_est, by = c("parameter", "sim_rep")) %>%
    select(parameter, sim_rep, model, mean_est1, mean_est2, CI_LL1, CI_LL2, CI_UL1, CI_UL2, sd_est)
...
save(ABM10_rm_nc_list, file = "ABM10_rm_nc_list.RData")
...
# summary
load("ABM10_hybrid_nc_list.RData")
load("ABM10_rm_nc_list.RData")

ABM10_boot_df_comp_nc <- bootstrap_summary(ABM10_rm_nc_list, true_val_NPI1 = -1.45, true_val_NPI2 = -0.8) %>% 
  bind_rows(bootstrap_summary(ABM10_hybrid_nc_list, true_val_NPI1 = -1.45, true_val_NPI2 = -0.8))

ggplot(ABM10_boot_df_comp_nc, aes(ymin = CI_LL2, ymax = CI_UL2, x = sim_rep, y = mean_est2, col = model)) + 
  geom_pointrange(position = position_dodge(width = 0.5)) + 
  scale_x_continuous(expand = c(0.01, 0.01)) + 
  facet_wrap(~parameter, ncol = 1, scale = "free_y") +
  geom_line(aes(y = true_value), linetype = "dashed", col = "darkred", linewidth = 0.8) + 
  labs(title= "Bootstrap ABM 10 new code", 
       x = "simulation dataset", y = "coefficient value") +
  theme_bw() +
  scale_color_brewer(palette = "Dark2")
...
ABM10_rm_nc_df_comp <- bootstrap_summary(ABM10_rm_nc_list, true_val_NPI1 = -1.45, true_val_NPI2 = -0.8) %>%
  mutate(model = "new code") %>%
  bind_rows(bootstrap_summary(ABM10_rm_list, true_val_NPI1 = -1.45, true_val_NPI2 = -0.8))


ggplot(ABM10_rm_nc_df_comp, aes(ymin = CI_LL2, ymax = CI_UL2, x = sim_rep, y = mean_est2, col = model)) + 
  geom_pointrange(position = position_dodge(width = 0.5)) + 
  scale_x_continuous(expand = c(0.01, 0.01)) + 
  facet_wrap(~parameter, ncol = 1, scale = "free_y") +
  geom_line(aes(y = true_value), linetype = "dashed", col = "darkred", linewidth = 0.8) + 
...
#### ABM code ####
ABM10_rm_ABMcode_list <- list()
ABM10_hybrid_ABMcode_list <- list()
ABM10_rm_ABMcode2_list <- list()
ABM10_hybrid_ABMcode2_list <- list()

for(j in 1:50){
  point_est_h <- read.table(paste0(getwd(), "/ABM_hybrid10_pe_ABMc_", j, "/populationParameters.txt"), 
                          header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j, model = "hybrid ABMcode") %>%
    rename(mean_est2 = value)
  
  point_est <- read.table(paste0(getwd(), "/ABM_rm10_pe_ABMc_", j, "/populationParameters.txt"), 
                            header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j, model = "rm ABMcode") %>%
    rename(mean_est2 = value)
  
  point_est_h2 <- read.table(paste0(getwd(), "/ABM_hybrid10_pe_ABMc2_", j, "/populationParameters.txt"), 
                            header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j, model = "hybrid ABMcode 2") %>%
    rename(mean_est2 = value)
  
  point_est2 <- read.table(paste0(getwd(), "/ABM_rm10_pe_ABMc2_", j, "/populationParameters.txt"), 
                          header = TRUE, sep = ",") %>%
    popparam_cleaning() %>%
    mutate(sim_rep = j, model = "rm ABMcode 2") %>%
    rename(mean_est2 = value)
  
  ABM10_rm_ABMcode_list[[j]] <- point_est
  ABM10_hybrid_ABMcode_list[[j]] <- point_est_h
  ABM10_rm_ABMcode2_list[[j]] <- point_est2
  ABM10_hybrid_ABMcode2_list[[j]] <- point_est_h2
}


ABM10_ABMcode_comp_df <- do.call("rbind.data.frame", ABM10_rm_ABMcode_list) %>%
  bind_rows(do.call("rbind.data.frame", ABM10_hybrid_ABMcode_list)) %>%
  bind_rows(do.call("rbind.data.frame", ABM10_rm_ABMcode2_list)) %>%
  bind_rows(do.call("rbind.data.frame", ABM10_hybrid_ABMcode2_list)) %>%
...
ggplot(ABM10_ABMcode_comp_df, aes(ymin = CI_LL2, ymax = CI_UL2, x = sim_rep, y = mean_est2, col = model)) + 
  geom_pointrange(position = position_dodge(width = 0.5)) + 
  scale_x_continuous(expand = c(0.01, 0.01), limits = c(1, 51)) + 
  facet_wrap(~parameter, ncol = 1, scale = "free_y") +
  geom_line(aes(y = true_value), linetype = "dashed", col = "darkred", linewidth = 0.8) + 
  labs(title= "Bootstrap ABM 10 new code", 
       x = "simulation dataset", y = "coefficient value") +
